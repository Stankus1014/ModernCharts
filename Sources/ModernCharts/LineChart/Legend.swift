...
struct Legend: View {
    ...
    var xLabels: [String] = ["SM","T","W","R","F","S"]
            ...
            ForEach((0...4), id: \.self) { height in
                HStack(alignment: .center){
                    ...
                    self.line(atHeight: self.getYLegendSafe(height: height), width: self.frame.width)
                        .stroke(style.legendTextColor, style: StrokeStyle(lineWidth: 1.5, lineCap: .round, dash: [5,height == 0 ? 0 : 5]))
...
struct Legend_Previews: PreviewProvider {
    static var previews: some View {
        GeometryReader{ geometry in
            Legend(data: ChartData(points: [212.2,214.5,213.5,213.9]), frame: .constant(geometry.frame(in: .local)), hideHorizontalLines: .constant(false))
